...
class InvoicePDF(FPDF):
    ...
    def header(self):
        ...
        self.cell(0, 10, self.store_name, new_x="LM", new_y="NEXT")
        self.set_font("helvetica", "", 9)
        self.set_text_color(110, 118, 135)
        self.cell(0, 5, self.support_email, new_x="LM", new_y="NEXT")
    ...
    def footer(self):
        ...
        self.cell(0, 5, f"Thank you for shopping with {self.store_name}!", align="C", new_x="LM", new_y="NEXT")
...
def generate_invoice(db: Session, order: Order, *, user_name: str, user_email: str) -> str:
    ...
    pdf.cell(0, 8, "TAX INVOICE", new_x="LM", new_y="NEXT")
    pdf.set_font("helvetica", "", 9)
    pdf.set_text_color(110, 118, 135)
    pdf.cell(0, 5, f"Invoice #{'INV-' + order.order_number}", new_x="LM", new_y="NEXT")
    ...
    pdf.cell(95, 6, "Billed / Shipped to", new_x="LM", new_y="NEXT")
    pdf.set_xy(105, y0)
    pdf.cell(0, 6, "Order details", new_x="LM", new_y="NEXT")

    pdf.set_font("helvetica", "", 9)
    addr = order.shipping_address or {}
    x = 105
    pdf.set_x(0)
    pdf.cell(95, 5, f"{user_name}", new_x="LM", new_y="NEXT")
    meta_rows = [
    ...
    for line in [addr.get("line1", ""), addr.get("line2") or "", f"{addr.get('city', '')}, {addr.get('state', '')} {addr.get('postal_code', '')}", addr.get("country", ""), f"Phone: {addr.get('phone', '')}"]:
        if line.strip():
            pdf.set_x(0)
            pdf.multi_cell(95, 5, line, new_x="LM", new_y="NEXT")
    ...
    for label, value in rows:
        ...
        pdf.cell(34, 7, f"{currency} {value:,.2f}", border=0, align="R", new_x="LM", new_y="NEXT")
